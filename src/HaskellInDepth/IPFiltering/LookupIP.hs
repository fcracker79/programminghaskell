module HaskellInDepth.IPFiltering.LookupIP where


import HaskellInDepth.IPFiltering.IPTypes ( IPRangeDB(..), IPRange(IPRange), IP )
...
lookupIP (IPRangeDB ips) ip = case find (inRange ip) ips of
    Nothing -> False
    Just _ -> True
    where inRange ip (IPRange beg end) = beg <= ip && ip <= end
