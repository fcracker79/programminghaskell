module HaskellProgrammingFromFirstPrinciples.MorraAlternative(playWithComputer, playWithFriend, MorraState, initialState) where
...
type PlayerAction = MaybeT (StateT MorraState IO) Int
...
msg :: Show a => a -> MaybeT (StateT MorraState IO) ()
msg = liftIO . print
...
increaseWinner :: Player -> Int -> String -> MaybeT (StateT MorraState IO) ()
...
playMorraM :: PlayersNames -> PlayerAction -> PlayerAction -> MaybeT (StateT MorraState IO) ()
playMorraM playersNames player1Choice player2Choice = do
    ...
    where player1Wins = increaseWinner Player1
          ...
          finalWinner state
                      | player1Score state >= rounds = do
                                                      msg $ fst playersNames ++ " is the final winner!"
                                                      empty
                      | player2Score state >= rounds = do
...
playMorra playersNames p1Choice p2Choice = do
    ...
    state <- execStateT (runMaybeT  (many (playMorraM playersNames p1Choice p2Choice))) is
    return $ if player1Score state > player2Score state then Player1 else Player2
